...
def main():
    try:
        ...
        print("%s %d %d %f" % ("Overall", totalCorrect,
                               totalCount, (float(totalCorrect) / totalCount)))
...
def getData(generateTuningData):
    ...
    for clazz in CLASSES:
        (train, tune, test) = buildTrainTestVectors(
            buildImageList(ROOT_DIR + clazz + "/"), generateTuningData)
...
def buildImageList(dirName):
    imgs = [Image.open(dirName + fileName).resize((DIMENSION, DIMENSION))
